(async function () {
    ...
    const nameInput = $('#first');
    const numberInput = $('#last');
    ...
    function showContactForm(contact) {
        if (contact) {
            firstNameInput.val(contact.name);
            lastNameInput.val(contact.number);
    ...
    addContactForm.submit(async event => {
        ...
        try {
            ...
            if (existingContact) {
                ...
                tds[0].textContent = newContact.name;
                tds[1].textContent = newContact.number;
